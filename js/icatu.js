...
<div id="game" class="tela flex flex-col justify-start items-center bg-[url(../img/bg-game.jpg)] bg-cover relative">
    ...
    <div id="questions" class="absolute top-[20%] w-[100%] max-w-[75%] border-4 border-[#1c2b5d]/25 rounded-[3rem] bg-white/50 flex flex-col justify-center items-center">
        ...
        <div id="answers" class="w-full flex flex-row justify-center items-center pb-4 px-8 gap-[3rem]">
            <button id="yes" data-option="0" class="w-[20%] border-8 border-[#1c2b5d]/25 bg-white/50 cursor-pointer flex flex-col justify-center items-center rounded-[3rem]"><p class="pointer-events-none text-[2rem] text-center font-bold text-[#1c2b5d]">sim</p></button>
            <button id="no" data-option="1" class="w-[20%] border-8 border-[#1c2b5d]/25 bg-white/50 cursor-pointer flex flex-col justify-center items-center rounded-[3rem]"><p class="pointer-events-none text-[2rem] text-center font-bold text-[#1c2b5d]">não</p></button>
        </div>
    </div>
...
function startGame() {
    if(questions[currentQuestion]) {
        ...
        for(let i in questions[currentQuestion].options) {
            if(questions[currentQuestion].options[i]==='sim') {select('#yes p').innerHTML = questions[currentQuestion].options[i]} 
            if(questions[currentQuestion].options[i]==='Icatu') {select('#yes p').innerHTML = questions[currentQuestion].options[i]}
            if(questions[currentQuestion].options[i]==='não') {select('#no p').innerHTML = questions[currentQuestion].options[i]} 
            if(questions[currentQuestion].options[i]==='não sei') {select('#no p').innerHTML = questions[currentQuestion].options[i]}
        }
